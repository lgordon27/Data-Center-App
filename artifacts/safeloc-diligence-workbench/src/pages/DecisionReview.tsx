...
export function DecisionReview({ onNavigate, onResolve, requestedAction, onRequestedActionHandled }: { onNavigate: (screen: Screen) => void; onResolve: (id: string) => void; requestedAction?: "save" | "compare" | null; onRequestedActionHandled?: () => void }) {
  ...
  return (
    <div>
      ...
      />
      {project.kind === "custom" && <div role="status" data-testid="custom-scenario-disabled" className="mb-5 rounded-lg border border-[#f1cb8b] bg-[#fff8e9] px-4 py-3 text-[11px] leading-5 font-semibold text-[#7f6337]">Named scenarios and comparisons are disabled for custom research. The active analysis is restored from this browser&apos;s local session; useful completed research may also remain in the local-server registry. Neither store is shared across devices.</div>}
      ...
      <div className="grid gap-5 xl:grid-cols-[0.9fr_1.1fr]">
        <section className={`motion-scale rounded-xl border border-[#d9e0e4] bg-[#122232] p-6 text-white transition-transform ${flash ? "scale-[1.01]" : ""}`} data-testid="panel-decision-return">
          <div className="flex items-start justify-between"><div><SectionKicker tone="lime" className="!text-[#d4e86b]">Primary synthetic current-evidence return</SectionKicker><div className="mt-2 text-[10px] uppercase tracking-[0.17em] text-[#a4b4bd]">Synthetic current-evidence project IRR</div></div><Gauge className="h-5 w-5 text-[#b9d43a]" /></div>
          ...
          <div className="mt-5 flex items-end justify-between gap-3">
           ...
           <div className="mt-5 border-t border-white/15 pt-4 text-[11px] leading-5 text-[#afbdc4]">Synthetic verified benchmark: <span className="font-mono text-white">{formatIRR(metrics.baseIRR ?? null)}</span>. The primary synthetic current-evidence case reflects evidence quality, timeline drag, and infrastructure risk. Optional EIA sensitivities do not set this recommendation.</div>
