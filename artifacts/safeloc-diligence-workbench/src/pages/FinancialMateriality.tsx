...
function EvidenceTraceButton({ inputId, testId, context, tone = "light" }: { inputId: string; testId: string; context: TraceContext; tone?: "light" | "dark" }) {
  ...
  return (
    <button
      data-testid={testId}
      type="button"
      aria-label={`Open financial effect trace for ${attribution.label}`}
      onClick={(event) => openDrawer({
        key: `financial-trace:${inputId}:${context}`,
        kicker: "Financial effect trace",
        title: `${attribution.label} — input trace`,
        render: () => (
          <div className="space-y-4">
            <DrawerSection label="Original and approved values">
              <div className="grid gap-2 sm:grid-cols-2">
                <DrawerField label="Raw model input" value={`${attribution.rawValue} ${attribution.rawUnit}`} />
                <DrawerField label="Applied model value" value={<span className="inline-flex flex-wrap items-center gap-1.5">{formatDriverValue(attribution.appliedValue, attribution.appliedUnit)} <ClassificationBadge value={attribution.currentClassification} compact /></span>} />
            ...
            <DrawerSection label="Source">
              <p>{attribution.citation}</p>
              <p className="font-mono text-[9px] uppercase tracking-[0.08em] text-[#7d898f]">Provenance: {attribution.provenance}</p>
            </DrawerSection>
...
export function FinancialMateriality({ onNavigate }: { onNavigate: (screen: Screen) => void }) {
  ...
  const currentIRRReason = metrics.projectIRRReason;
  const baseIRRReason = metrics.baseModel?.projectIRRReason ?? null;
  const irrDelta = currentIRR === null || baseIRR === null ? null : currentIRR - baseIRR;
  const scenarioIRRReasons = Array.from(new Set(
    metrics.returnSensitivity
      .filter((scenario) => scenario.irr === null)
      .map((scenario) => scenario.irrReason)
