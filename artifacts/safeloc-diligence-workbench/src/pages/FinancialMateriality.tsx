...
import { ArrowRight, ChevronDown, FileSearch, Sparkles, TrendingDown, TrendingUp, TriangleAlert } from "lucide-react";
import {
  BottomNav,
  ClassificationBadge,
  ImpactRoleBadge,
...
function NextViewButton({ label, onClick }: { label: string; onClick: () => void }) {
  return <button type="button" onClick={onClick} className="mt-5 inline-flex min-h-11 items-center gap-2 rounded-md bg-[#122232] px-4 text-xs font-semibold text-[#d4e86b] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#b9d43a]">Next: {label}<ArrowRight aria-hidden="true" className="h-4 w-4" /></button>;
}
...
export function FinancialMateriality({ onNavigate }: { onNavigate: (screen: Screen) => void }) {
  ...
  const materialAttributions = rankedAttributions.filter((item) => (item.singleInputSensitivityIRR ?? 0) !== 0);
  const immaterialAttributions = rankedAttributions.filter((item) => (item.singleInputSensitivityIRR ?? 0) === 0);
    ...
    <div data-testid="financial-transmission-model" className="min-w-0">
      ...
      {financialView === "overview" && <section id="financial-panel-overview" data-testid="panel-impact-chain" role="tabpanel" aria-labelledby="financial-tab-overview" tabIndex={0} className="rounded-xl border-2 border-[#122232] bg-[#122232] p-5 text-white md:p-6">
        ...
        <div className="mt-5 grid gap-2 md:grid-cols-4">
          {[
            ["01", "Physical constraint", "Power, water, grid, construction or community conditions."],
            ["02", "Project economics", "Timing, capital cost and operating effects enter the project model."],
            ["03", "Issuer exposure", originatingCompany ? `${originatingCompany} exposure depends on documented contracts and financial dependence.` : "No issuer effect is attributed without a documented relationship."],
            ["04", "Portfolio / advisor question", "Test actual holding weight, issuer dependence and the client decision."],
        ...
        <NextViewButton label="Key Drivers" onClick={() => setFinancialView("key-drivers")} />
      ...
      {financialView === "key-drivers" && <section id="financial-panel-key-drivers" role="tabpanel" aria-labelledby="financial-tab-key-drivers" tabIndex={0} className="space-y-4">
        <div className="rounded-xl border border-[#d9e0e4] bg-white p-5 md:p-6"><SectionKicker>Ranked financial effects</SectionKicker><h3 className="text-[21px] font-semibold tracking-[-0.03em] text-[#122232]">What moves the conservative return?</h3><p className="mt-2 text-[11px] leading-5 text-[#52616b]">Ranked by absolute single-input IRR sensitivity. Each row shows direction, magnitude, evidence status and the treatment behind it.</p>
          <div className="mt-4 divide-y divide-[#e5eae8]">
            {materialAttributions.map((attribution, index) => {
              ...
              return <article key={attribution.id} data-testid={`impact-chain-row-${attribution.id}`} className="grid gap-3 py-4 lg:grid-cols-[2rem_1.2fr_0.7fr_0.8fr_1.5fr_auto] lg:items-center">
                ...
                <div><div className="text-[8px] font-bold uppercase tracking-[0.1em] text-[#7d898f]">Evidence / treatment</div><div className="mt-1"><span data-testid={`impact-chain-classification-${attribution.id}`} className="sr-only">{item.classification}</span><ClassificationBadge value={item.classification} compact /></div><p className="mt-1 text-[9px] leading-4 text-[#52616b]">{attribution.currentTreatment}</p></div>
                <EvidenceTraceButton inputId={attribution.id} testId={`button-trace-impact-chain-${attribution.id}`} context="row" />
              </article>;
            })}
          </div>
          {immaterialAttributions.length > 0 && <details data-testid="disclosure-immaterial-drivers" className="mt-4 rounded-lg border border-[#d9e0e4] bg-[#f7faf8]"><summary className="cursor-pointer list-none px-4 py-3 text-[11px] font-semibold text-[#52616b] [&::-webkit-details-marker]:hidden">{immaterialAttributions.length} immaterial effects at the current classification <ChevronDown aria-hidden="true" className="ml-2 inline h-4 w-4" /></summary><div className="border-t border-[#d9e0e4] p-4">{immaterialAttributions.map((attribution) => <div key={attribution.id} className="flex flex-wrap items-center justify-between gap-2 border-b border-[#e5eae8] py-2 text-[10px] last:border-0"><span>{evidence[attribution.id].label}</span><span>No adjustment at current classification</span></div>)}</div></details>}
        </div>
        ...
        <NextViewButton label="Cash Flows" onClick={() => setFinancialView("cash-flows")} />
      </section>}
      ...
      {financialView === "cash-flows" && <section id="financial-panel-cash-flows" data-testid="financial-panel-cash-flows" role="tabpanel" aria-labelledby="financial-tab-cash-flows" tabIndex={0} className="space-y-5">
        <div id="materiality-summary" className="grid gap-3 sm:grid-cols-2 xl:grid-cols-5"><MetricCard testId="metric-project-irr" label="Project IRR" value={formatIRR(currentIRR)} detail={`${formatPercentagePoints(irrDelta, { signed: true })} vs baseline`} accent="lime" /><MetricCard testId="metric-moic" label="MOIC" value={formatScenarioMetric(metrics.moic, "moic")} detail="5-year hold period" accent="navy" /><MetricCard testId="metric-coc" label="Cash-on-cash" value={formatScenarioMetric(metrics.cashOnCash, "cashOnCash")} detail="Stabilized year 3" accent="violet" /><MetricCard testId="metric-payback" label="Payback" value={formatPayback(metrics.payback)} detail="Cumulative equity breakeven" accent="coral" /><MetricCard testId="metric-npv" label="NPV @ 10%" value={formatCurrency(metrics.npv, 0)} detail="Equity value created" accent="navy" /></div>
        <section data-testid="panel-cash-flow-comparison" className="rounded-xl border border-[#d9e0e4] bg-white p-5 md:p-6"><SectionKicker>Cash-flow comparison</SectionKicker><h3 className="text-[19px] font-semibold text-[#122232]">Baseline and stress equity cash flows</h3><div className="mt-4 max-w-full overflow-x-auto rounded border border-[#d9e0e4]"><table className="w-full min-w-[760px] border-collapse text-left"><caption className="sr-only">Underwriting baseline and conservative stress equity cash flows in millions of dollars</caption><thead className="sticky top-0 bg-[#f1f5f3] text-[9px] uppercase text-[#52616b]"><tr><th scope="col" className="sticky left-0 bg-[#f1f5f3] px-3 py-3">Period</th><th scope="col" className="px-3 py-3">Baseline equity CF ($M)</th><th scope="col" className="px-3 py-3">Stress equity CF ($M)</th><th scope="col" className="px-3 py-3">Marginal delta ($M)</th><th scope="col" className="px-3 py-3">Treatment</th></tr></thead><tbody className="divide-y divide-[#e5eae8] font-mono text-[10px]">{metrics.schedule.map((year, index) => { const baselineYear = metrics.baseModel?.schedule[index]; const delta = year.netEquityCashFlow - (baselineYear?.netEquityCashFlow ?? year.netEquityCashFlow); return <tr key={year.year} data-testid={`cash-flow-comparison-y${year.year}`}><th scope="row" className="sticky left-0 bg-white px-3 py-3 text-[#122232]">{year.year === 0 ? "Close / Year 0" : `Year ${year.year}`}</th><td className="px-3 py-3">{formatCurrency(baselineYear?.netEquityCashFlow ?? 0)}</td><td className="px-3 py-3">{formatCurrency(year.netEquityCashFlow)}</td><td className="px-3 py-3">{formatCurrency(delta)}</td><td className="px-3 py-3 font-sans text-[9px]">{year.year === 0 ? "Initial equity funding" : year.year === 5 ? "Operations, exit proceeds and debt repayment" : "Operating NOI less debt service"}</td></tr>; })}</tbody></table></div></section>
        <section className="rounded-xl border border-[#d9e0e4] bg-white p-5 md:p-6"><SectionKicker>Complete annual schedule</SectionKicker><h3 className="text-[19px] font-semibold text-[#122232]">Five-year project-level cash-flow schedule</h3><div className="mt-4 max-w-full overflow-x-auto rounded border border-[#d9e0e4]"><table className="w-full min-w-[900px] border-collapse text-left"><caption className="sr-only">Five-year annual project cash-flow schedule in millions of dollars</caption><thead className="sticky top-0 bg-[#f1f5f3] text-[9px] uppercase text-[#52616b]"><tr><th scope="col" className="sticky left-0 bg-[#f1f5f3] px-3 py-3">Year</th><th scope="col" className="px-3 py-3">Revenue ($M)</th><th scope="col" className="px-3 py-3">Climate OPEX ($M)</th><th scope="col" className="px-3 py-3">NOI ($M)</th><th scope="col" className="px-3 py-3">Debt service ($M)</th><th scope="col" className="px-3 py-3">Terminal value ($M)</th><th scope="col" className="px-3 py-3">Net equity CF ($M)</th><th scope="col" className="px-3 py-3">Cumulative CF ($M)</th></tr></thead><tbody className="divide-y divide-[#e5eae8] font-mono text-[10px]">{metrics.schedule.map((year) => <tr key={year.year}><th scope="row" className="sticky left-0 bg-white px-3 py-3 text-[#122232]">{year.year === 0 ? "Close" : `Y${year.year}`}</th><td className="px-3 py-3">{formatCurrency(year.revenue)}</td><td data-testid={`text-climate-opex-y${year.year}`} className="px-3 py-3">{year.year === 0 ? "—" : formatCurrency(year.climateDisruptionOpex)}</td><td className="px-3 py-3">{formatCurrency(year.noi)}</td><td className="px-3 py-3">{formatCurrency(year.interest + year.principal)}</td><td className="px-3 py-3">{year.terminalValue ? formatCurrency(year.terminalValue) : "—"}</td><td className="px-3 py-3">{formatCurrency(year.netEquityCashFlow)}</td><td className="px-3 py-3">{formatCurrency(year.cumulativeEquityCashFlow)}</td></tr>)}</tbody></table></div></section>
        <NextViewButton label="Assumptions" onClick={() => setFinancialView("assumptions")} />
      </section>}
      ...
      {financialView === "assumptions" && <section id="financial-panel-assumptions" data-testid="financial-panel-assumptions" role="tabpanel" aria-labelledby="financial-tab-assumptions" tabIndex={0} className="space-y-4">
        ...
        <button type="button" onClick={() => onNavigate("advisor")} className="inline-flex min-h-11 items-center gap-2 rounded-md bg-[#122232] px-4 text-xs font-semibold text-[#d4e86b]">Next: Advisor Brief <ArrowRight aria-hidden="true" className="h-4 w-4" /></button>
      </section>}
